...
import com.app.digiposfinalapp.util.DemoSleeper;
...
public class MainActivity3 extends AppCompatActivity {
    ...
    private void sendTestLabel() {
        try {
            ...
            String inputText = getUserInputText();

            // Create a command to print the input text on the label
            byte[] configLabel = getConfigLabel(inputText);
     ...
     *
     */
    private byte[] getConfigLabel(String inputText) {
        PrinterLanguage printerLanguage = printer.getPrinterControlLanguage();
        byte[] configLabel = null;

        if (printerLanguage == PrinterLanguage.ZPL) {
            // Create a ZPL command to print the user input text
            configLabel = ("^XA^FO17,16^GB379,371,8^FS^FT65,255^A0N,135,134^FD" + inputText + "^FS^XZ").getBytes();
        } else if ((printerLanguage == PrinterLanguage.CPCL) || (printerLanguage == PrinterLanguage.LINE_PRINT)) {
            // Create a CPCL command to print the user input text
            String cpclConfigLabel = "! 0 200 200 406 1\r\n" + "ON-FEED IGNORE\r\n" + "BOX 20 20 380 380 8\r\n" +
                    "T 0 6 137 177 " + inputText + "\r\n" + "PRINT\r\n";
